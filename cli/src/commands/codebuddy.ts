...
import { CODEBUDDY_PERMISSION_MODES } from '@hapi/protocol/modes'
        ...
        try {
            ...
            for (let i = 0; i < commandArgs.length; i++) {
                ...
                if (arg === '--started-by') {
                    ...
                    if (!mode || !(CODEBUDDY_PERMISSION_MODES as readonly string[]).includes(mode)) {
                        throw new Error(`Invalid --permission-mode value: ${mode ?? '(missing)'}`)
                    }
                    options.permissionMode = mode as CodebuddyPermissionMode
                    hasExplicitPermissionMode = true
                } else if ((arg === '--yolo' || arg === '--dangerously-skip-permissions') && !hasExplicitPermissionMode) {
