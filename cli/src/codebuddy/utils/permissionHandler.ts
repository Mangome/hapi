...
function deriveToolInput(request: PermissionRequest): unknown {
    if (request.rawInput !== undefined) return request.rawInput;
...
export class CodebuddyPermissionHandler extends BasePermissionHandler<PermissionResponseMessage, void> {
    private readonly pendingBackendRequests = new Map<string, PermissionRequest>();

    constructor(
        session: ApiSessionClient,
        private readonly backend: AgentBackend,
        private readonly getPermissionMode: () => CodebuddyPermissionMode | undefined
    ) {
        super(session);
    ...
    private handlePermissionRequest(request: PermissionRequest): void {
        ...
        logger.debug(`[CodeBuddy] Permission request queued for ${toolName} (${request.id})`);
    }
    ...
    private async autoApprove(
        ...
        logger.debug(`[CodeBuddy] Auto-approved ${toolName} (${request.id}) mode=${decision}`);
    }
    ...
    protected async handlePermissionResponse(
        ...
        if (pendingRequest) {
            ...
            logger.debug('[CodeBuddy] Permission response missing backend request', response.id);
        }
        ...
        logger.debug(`[CodeBuddy] Permission ${response.approved ? 'approved' : 'denied'} for ${pending.toolName}`);
    ...
    protected handleMissingPendingResponse(response: PermissionResponseMessage): void {
        logger.debug('[CodeBuddy] Permission response received for unknown request', response.id);
    }
